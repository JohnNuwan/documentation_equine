...
def architecte(state: ChapterState) -> ChapterState:
    """Agent Architecte : Planification du chapitre"""
...
Crée un plan structuré avec des sections et sous-sections claires."""
    ...
    try:
        ...
        console.print(f"  [green]✓ Plan créé[/green]")
        return {"plan_chapitre": str(plan)}
    except Exception as e:
        console.print(f"  [red]✗ Erreur planification: {e}[/red]")
        return {"plan_chapitre": "Erreur lors de la planification"}

def chercheur_web(state: ChapterState) -> ChapterState:
    """Agent Chercheur : Recherche web d'informations complémentaires"""
    console.print(f"[yellow]2. Agent Chercheur:[/yellow] Recherche web pour {state['nom_fichier']}...")
    
    try:
        # Recherche sur le titre du fichier
        sujet_recherche = state['nom_fichier'].replace('.md', '').replace('_', ' ').replace('/', ' ').replace('\\', ' ')
        console.print(f"  [cyan]Recherche sur: {sujet_recherche}[/cyan]")
        
        # Recherche web avec DuckDuckGo
        query = f"recherche scientifique et vétérinaire détaillée sur {sujet_recherche} équitation cheval"
        resultats = search_tool.run(query)
        
        console.print(f"  [green]✓ Recherche web terminée[/green]")
        return {"recherche_web": str(resultats)}
        
    except Exception as e:
        console.print(f"  [yellow]⚠ Erreur recherche web: {e}[/yellow]")
        return {"recherche_web": f"Recherche web non disponible pour {state['nom_fichier']}. Utilisation du contenu source uniquement."}

def validateur(state: ChapterState) -> ChapterState:
    """Agent Validateur : Validation et synthèse des informations"""
    console.print(f"[yellow]3. Agent Validateur:[/yellow] Validation pour {state['nom_fichier']}...")
    
    prompt = f"""Tu es un scientifique et vétérinaire rigoureux. Analyse le plan du chapitre, le contenu source et les informations de recherche web. 
    Croise les sources, vérifie les faits et produis une synthèse neutre, factuelle et fiable qui servira de base à la rédaction.
...
SYNTHÈSE VALIDÉE:"""
    ...
    try:
        ...
        console.print(f"  [green]✓ Synthèse validée[/green]")
        return {"synthese_validee": str(synthese)}
    except Exception as e:
        console.print(f"  [red]✗ Erreur validation: {e}[/red]")
        return {"synthese_validee": "Erreur lors de la validation"}

def redacteur(state: ChapterState) -> ChapterState:
    """Agent Rédacteur : Rédaction du chapitre complet"""
    console.print(f"[yellow]4. Agent Rédacteur:[/yellow] Rédaction de {state['nom_fichier']}...")
...
[Résumé des points clés]"""
    ...
    try:
        ...
        console.print(f"  [green]✓ Chapitre rédigé[/green]")
        return {"texte_redige": str(texte)}
    except Exception as e:
        console.print(f"  [red]✗ Erreur rédaction: {e}[/red]")
        return {"texte_redige": "Erreur lors de la rédaction"}

def illustrateur(state: ChapterState) -> ChapterState:
    """Agent Illustrateur : Création des schémas et illustrations"""
    console.print(f"[yellow]5. Agent Illustrateur:[/yellow] Création des schémas pour {state['nom_fichier']}...")
    
    texte = state['texte_redige']
    placeholders = re.findall(r'\[SCHEMA: (.*?)\]', texte)
    
    if not placeholders:
        console.print(f"  [cyan]Aucun schéma demandé[/cyan]")
...
Ne fournis que le code DOT, sans explications."""
        ...
        try:
            ...
            try:
                result = os.system(f'"C:\\Program Files\\Graphviz\\bin\\dot.exe" -Tpng {dot_path} -o {png_path}')
                if result == 0:
...
def relecteur(state: ChapterState) -> ChapterState:
    """Agent Relecteur : Relecture et finalisation du chapitre"""
    console.print(f"[yellow]6. Agent Relecteur:[/yellow] Relecture de {state['nom_fichier']}...")
...
Corrige et améliore ce texte en gardant la même structure et longueur."""
    ...
    try:
        ...
        console.print(f"  [green]✓ Relecture terminée[/green]")
        return {"texte_final": str(texte_final)}
    except Exception as e:
        console.print(f"  [red]✗ Erreur relecture: {e}[/red]")
        return {"texte_final": state['texte_avec_schemas']}
...
workflow.add_node("chercheur_web", chercheur_web)
...
workflow.add_edge("architecte", "chercheur_web")
workflow.add_edge("chercheur_web", "validateur")
...
console.print("[bold green]Graphe Académique d'Excellence (6 agents) prêt ![/bold green]")
...
def main():
    console.print("\n[bold cyan]=== CRÉATION DU LIVRE ACADÉMIQUE D'EXCELLENCE ===[/bold cyan]")
    console.print("[cyan]Utilisation de LangChain + LangGraph pour une performance optimale[/cyan]")
    ...
    # Traiter chaque chapitre avec le graphe LangGraph optimisé
    ...
    try:
        with open('livre_academique_excellence.md', 'w', encoding='utf-8') as f:
            f.write("\n\n".join(livre_final))
        
        console.print("\n[bold green]========================================[/bold green]")
        console.print("[bold green]LIVRE ACADÉMIQUE D'EXCELLENCE CRÉÉ ![/bold green]")
        console.print("[bold green]========================================[/bold green]")
        console.print(f"[green]Le livre a été sauvegardé dans : [bold]livre_academique_excellence.md[/bold][/green]")
        console.print(f"[green]Contenu basé sur {len(fichiers_md)} fichiers .md enrichis[/green]")
        console.print(f"[green]Chaque chapitre a été enrichi avec des recherches web et des schémas[/green]")
        console.print(f"[green]Architecture optimisée : LangChain + LangGraph[/green]")
